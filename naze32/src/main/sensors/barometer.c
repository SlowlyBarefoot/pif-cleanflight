...
uint32_t baroUpdate(PifTask *p_task)
{
    ...
    switch (state) {
        ...
        case BAROMETER_NEEDS_CALCULATION:
            ...
            if (baroReady && !p_task->_running) pifTask_SetTrigger(p_task);
...
void evtBaroRead(float pressure, float temperature)
{
    ...
    baroPressureSum = recalculateBarometerTotal(barometerConfig->baro_sample_count, baroPressureSum, baroPressure);
    if (baroReady && !cfTasks[TASK_ALTITUDE].p_task->_running) pifTask_SetTrigger(cfTasks[TASK_ALTITUDE].p_task);
}
