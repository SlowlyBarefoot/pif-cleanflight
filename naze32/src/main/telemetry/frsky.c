...
extern int32_t telemTemperature1; // FIXME dependency on mw.c
...
#ifdef BARO
    serialize16((int16_t)(sensor_link.baro.temperature + 0.5f)); //Airmamaf
#else
...
void configureFrSkyTelemetryPort(void)
{
    ...
    frskyPort = openSerialPort(portConfig->identifier, FUNCTION_TELEMETRY_FRSKY, NULL, FRSKY_BAUDRATE, FRSKY_INITIAL_PORT_MODE, telemetryConfig->telemetry_inversion ? SERIAL_INVERTED : SERIAL_NOT_INVERTED, 10);
    if (!frskyPort) {
        return;
    }
